...
void PipeWireProduce::setupStream()
{
    ...
    m_passthroughThread = std::thread([this]() {
        ...
        while (m_passthroughRunning) {
            ...
            // If we're deactivating but still have frames in the queue, we want
            // to flush everything. Since at that point we are not receiving new
            // frames, waiting here would prevent any remaining frames from being
            // processed, so instead just spin the filter thread to ensure all
            // remaining things are processed.
            if (!m_deactivated) {
